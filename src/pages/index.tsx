...
const ActionArea = () => {
  ...
  const toSignnewfile = () => push("/signnewfile")
  return (
    <div className="absolute right-[15%] md:right-[6%] top-1/4">
      <div className="flex flex-col w-[468px] jusifty-center items-center">
        ...
        <div className="w-[468px] h-[100px]">
          <BiggestButton action={toSignnewfile} text="簽署新文件" isDisabled={false} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col justify-between h-full items-center py-20">
      <div className="px-20">
        ...
        <span className="text-secondary md:text-xs xs:text-xs">
          線上簽署，方便快速。
        </span>
      </div>
      <div className="px-20 w-full h-10">
        <BiggestButton action={() => push("/signnewfile")} text="簽署新文件" isDisabled={false} />
      </div>
    </div>
  );
};
